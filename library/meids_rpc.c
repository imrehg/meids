...
static int RPC_init(void)
{
	...
	cfg_err = config_list_init(&RPC_Config);
	cnx_err = context_list_init(&RPC_Context);
	rpc_err = rpc_init_calltable(&Rpc_Calls);

	if (cnx_err  || cfg_err || rpc_err)
	{
		if (RPC_Context)
			free(RPC_Context);
...
int ME_Open(char* address, int iFlags)
{
	...
	else
	{
		...
		{
			err_RPC = ME_rpc_OpenDriver("192.168.20.228", &cfg, &context, ME_OPEN_RPC);
			if (!err_RPC)
			{
				new_global_cfg = NULL;
				ConfigJoin(cfg, RPC_Config, &new_global_cfg, ME_VALUE_NOT_USED);
		...
		err = (err_RPC) ? ME_ERRNO_NOT_OPEN : ME_ERRNO_SUCCESS;
	}
		...
		for (i=0; i<RPC_Config->device_list_count; i++, device_list++)
		{
			...
			switch ((*device_list)->access_type)
			{
				case me_access_type_TCPIP:						//Synapse-LAN
					err = ME_rpc_CloseDriver((*device_list)->context, iFlags);
					break;
...
int ME_QueryDevicesNumber(int* no_devices, int iFlags)
{
	...
	return ConfigMaxNumber(RPC_Config, no_devices, ME_VALUE_NOT_USED);
}
...
int ME_ConfigRead(me_config_t* cfg, const char* address, int flags)
{
	...
	if (!err)
	{
		err = ConfigRead_RPC(context, cfg, address, flags);
...
{
	return ME_virtual_QuerySubdevicesNumberByType(RPC_Config, device, type, subtype, no_subdevice, iFlags);
}
...
{
	return ME_virtual_IrqTest(RPC_Config, device, subdevice, channel, iFlags);
}

int  ME_IrqSetCallback(int device, int subdevice, meIOIrqCB_t irq_fn, void* irq_context, int iFlags)
{
	return ME_virtual_IrqSetCallback(RPC_Config, device, subdevice, irq_fn, irq_context, iFlags);
}
...
{
	return ME_virtual_StreamStop(RPC_Config, device, subdevice, mode, timeout, iFlags);
}
...
int  ME_StreamSetCallbacks(int device, int subdevice,
							...
							int iFlags)
{
	return ME_virtual_StreamSetCallbacks(RPC_Config,
							device, subdevice,
							start, start_context,
...
{
	return ME_virtual_StreamFrequencyToTicks(RPC_Config, device, subdevice, timer, frequency, ticks_low, ticks_high, iFlags);
}

int  ME_ParametersSet(me_extra_param_set_t* paramset, int flags)
{
	return ME_virtual_ParametersSet(RPC_Config, paramset, flags);
}

int  ME_SetOffset(int device, int subdevice, int channel, int range, double* offset, int iFlags)
{
	return ME_virtual_SetOffset(RPC_Config, device, subdevice, channel, range, offset, iFlags);
}


void ME_ConfigPrint(void)
{
	LIBPDEBUG("RPC_Config=%p\n", RPC_Config);
	LIBPDEBUG("RPC_Config->device_list_count=%d\n", RPC_Config->device_list_count);
	LIBPDEBUG("RPC_Config->device_list=%p\n", RPC_Config->device_list);
