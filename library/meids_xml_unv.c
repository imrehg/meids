/* Shared library for Meilhaus driver system (local + XML).
 * ==========================================
 *
...
# include "meids_xml_unv.h"

static int unv_xml_init(void);
...
static me_config_t*					Unv_Config_Raw;
...
void __attribute__((constructor)) meids_unv_xml_init(void);
void __attribute__((destructor)) meids_unv_xml_fini(void);


/// Private section
void meids_unv_xml_init(void)
{
	LIBPINFO("executed: %s\n", __FUNCTION__);
...
static int unv_xml_init(void)
{
	...
	cfg_raw_err = config_list_init(&Unv_Config_Raw);
	...
	if (cnx_err || cfg_raw_err  || cfg_err || loc_err || rpc_err)
	{
...
int ME_Open(char* address, int iFlags)
{
	...
	if (address)
	{
		LIBPDEBUG("Execute Open(%s)\n", address);
		err = ME_unv_xml_OpenDriver(address, &cfg, &context, iFlags);
		if (!err)
		{
			new_global_cfg = NULL;
			ConfigJoin(Unv_Config_Raw, cfg, &new_global_cfg, ME_VALUE_NOT_USED);
			ConfigClean(Unv_Config_Raw, ME_VALUE_NOT_USED);
			...
			Unv_Config_Raw->device_list_count = new_global_cfg->device_list_count;
			Unv_Config_Raw->device_list = new_global_cfg->device_list;
			if (new_global_cfg)
			{
				free(new_global_cfg);
				new_global_cfg = NULL;
			...
			ConfigEnumerate(Unv_Config_Raw, 0, ME_VALUE_NOT_USED);
	...
	{/// @note No address provided. USEING DEFAULT VALUES! Only local devices!
		...
		{
			err_PCI = ME_unv_xml_OpenDriver("/dev/medriverPCI", &cfg, &context, ME_OPEN_PCI);
			if (!err_PCI)
			{
				new_global_cfg = NULL;
				ConfigJoin(Unv_Config_Raw, cfg, &new_global_cfg, ME_VALUE_NOT_USED);
		...
		{
			err_USB = ME_unv_xml_OpenDriver("/dev/medriverUSB", &cfg, &context, ME_OPEN_USB);
			if (!err_USB)
			{
				new_global_cfg = NULL;
				ConfigJoin(Unv_Config_Raw, cfg, &new_global_cfg, ME_VALUE_NOT_USED);
		...
		err = (err_PCI && err_USB) ? ME_ERRNO_OPEN : ME_ERRNO_SUCCESS;
	}

	if (!err)
	{
		ConfigLoad_XML_ALL(&cfg_XML, MEIDS_XML_FILE, ME_VALUE_NOT_USED);

		ConfigClean(Unv_Config, ME_VALUE_NOT_USED);
		ConfigDuplicate(Unv_Config_Raw, &Tmp_Config, ME_VALUE_NOT_USED);

		ConfigBind(&cfg_XML, Tmp_Config, Unv_Config, ME_VALUE_NOT_USED);

		ConfigClean(Tmp_Config, ME_VALUE_NOT_USED);
		free(Tmp_Config);
		Tmp_Config = NULL;
		...
		for (i=0; i<Unv_Config->device_list_count; i++, device_list++)
		{
			...
			switch ((*device_list)->access_type)
			{
				...
				case me_access_type_TCPIP:						//Synapse-LAN
					err = ME_unv_xml_CloseDriver((*device_list)->context, iFlags);
					break;
