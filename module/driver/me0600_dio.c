 ...
 * @file me0600_dio.c
 *
 * @brief The ME-0600 (ME-630) digital input/output subdevice instance.
...
int me0600_dio_io_reset_subdevice(me_subdevice_t* subdevice, struct file* filep, int flags);
int me0600_dio_io_single_config(me_subdevice_t* subdevice, struct file* filep, int channel,
										int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags);
int me0600_dio_io_single_read(me_subdevice_t* subdevice, struct file* filep, int channel, int* value, int time_out, int flags);
int me0600_dio_io_single_write(me_subdevice_t* subdevice, struct file* filep, int channel, int value, int time_out, int flags);
int me0600_dio_query_number_channels(me_subdevice_t* subdevice, int* number);
int me0600_dio_query_subdevice_type(me_subdevice_t* subdevice, int* type, int* subtype);
int me0600_dio_query_subdevice_caps(me_subdevice_t* subdevice, int* caps);
	...
	ME_SUBDEVICE_ENTER;
		ME_SUBDEVICE_LOCK;
			ME_SPIN_LOCK(instance->ctrl_reg_lock);
				...
				tmp  &= ~(0x3 << (instance->base.idx * ME0600_DIO_MODE_SHIFT));
	...
	ME_SUBDEVICE_ENTER;
		ME_SUBDEVICE_LOCK
			ME_SPIN_LOCK(instance->ctrl_reg_lock);
				...
				{
					case ME_SINGLE_CONFIG_DIO_INPUT:
						mode &= ~((ME0600_DIO_CONFIG_BIT_OUT_0) << (instance->base.idx * ME0600_DIO_MODE_SHIFT));
						break;
...
int me0600_dio_io_single_write(me_subdevice_t* subdevice, struct file* filep, int channel, int value, int time_out, int flags)
{
	...
	ME_SUBDEVICE_ENTER;
		ME_SUBDEVICE_LOCK;
			me_readb(instance->base.dev, &mode, instance->ctrl_reg);
			if (!(mode & (ME0600_DIO_CONFIG_BIT_OUT_0 << (instance->base.idx * ME0600_DIO_MODE_SHIFT))))
			{
...
int me0600_dio_query_number_channels(me_subdevice_t* subdevice, int* number)
{
	PDEBUG("executed idx=%d.\n", ((me0600_dio_subdevice_t *) subdevice)->base.idx);
...
me0600_dio_subdevice_t* me0600_dio_constr(void* reg_base, unsigned int idx, me_lock_t* ctrl_reg_lock)
{
	...
	subdevice = kzalloc(sizeof(me0600_dio_subdevice_t), GFP_KERNEL);
	...
	subdevice->port_reg = reg_base + ME0600_DIO_PORT_REG + idx;

	// Overload base class methods.
	subdevice->base.me_subdevice_io_reset_subdevice = me0600_dio_io_reset_subdevice;
	subdevice->base.me_subdevice_io_single_config = me0600_dio_io_single_config;
	subdevice->base.me_subdevice_io_single_read = me0600_dio_io_single_read;
	subdevice->base.me_subdevice_io_single_write = me0600_dio_io_single_write;
	subdevice->base.me_subdevice_query_number_channels = me0600_dio_query_number_channels;
	subdevice->base.me_subdevice_query_subdevice_type = me0600_dio_query_subdevice_type;
	subdevice->base.me_subdevice_query_subdevice_caps = me0600_dio_query_subdevice_caps;
