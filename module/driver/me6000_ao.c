 ...
 * @file me6000_ao.c
 *
 * @brief The ME-6000 analog output subdevice instance.
...
int me6000_ao_query_range_by_min_max(me_subdevice_t* subdevice, int unit, int* min, int* max, int* maxdata, int* range);
int me6000_ao_query_number_ranges(me_subdevice_t* subdevice, int unit, int* count);
int me6000_ao_query_range_info(me_subdevice_t* subdevice, int range, int* unit, int* min, int* max, int* maxdata);
int me6000_ao_query_timer(me_subdevice_t* subdevice, int timer, int* base_frequency, uint64_t* min_ticks, uint64_t* max_ticks);
int me6000_ao_query_number_channels(me_subdevice_t* subdevice, int* number);
int me6000_ao_query_subdevice_type(me_subdevice_t* subdevice, int* type, int* subtype);
int me6000_ao_query_subdevice_caps(me_subdevice_t* subdevice, int* caps);
int me6000_ao_query_subdevice_caps_args(me_subdevice_t* subdevice, int cap, int* args, int* count);

int me6000_ao_postinit(me_subdevice_t* subdevice, void* args);

/// Remove subdevice.
static void me6000_ao_destructor(me_subdevice_t* subdevice);
/// Reset subdevice. Stop all actions. Reset registry. Disable FIFO. Set output to 0V and status to 'none'.
int me6000_ao_io_reset_subdevice(me_subdevice_t* subdevice, struct file* filep, int flags);
/// Set output as single*/
static int me6000_ao_io_single_config_check(me6000_ao_subdevice_t* instance, int channel,
    									int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags);
int me6000_ao_io_single_config(me_subdevice_t* subdevice, struct file* filep, int channel,
    									int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags);
/// Pass to user actual value of output.
static int me6000_ao_io_single_read_check(me6000_ao_subdevice_t* instance, int channel, int* value, int time_out, int flags);
int me6000_ao_io_single_read(me_subdevice_t* subdevice, struct file* filep, int channel,  int* value, int time_out, int flags);
/// Write to output requed value.
static int me6000_ao_io_single_write_check(me6000_ao_subdevice_t* instance, int channel, int value, int time_out, int flags);
int me6000_ao_io_single_write(me_subdevice_t* subdevice, struct file* filep, int channel, int value, int time_out, int flags);
/// Set output as streamed device.
static int me6000_ao_io_stream_config_check(me6000_ao_subdevice_t* instance,
    									meIOStreamSimpleConfig_t* config_list, int count, meIOStreamSimpleTriggers_t* trigger, int fifo_irq_threshold, int flags);
int me6000_ao_io_stream_config(me_subdevice_t* subdevice, struct file* filep,
										meIOStreamSimpleConfig_t* config_list, int count,  meIOStreamSimpleTriggers_t* trigger, int fifo_irq_threshold, int flags);
/// Helper for 'ao_io_stream_new_values'. Check HF bit.
inline static int me6000_ao_FSM_test(me6000_ao_subdevice_t* instance);
/// Wait for / Check empty space in buffer.
int me6000_ao_io_stream_new_values(me_subdevice_t* subdevice, struct file* filep, int time_out, int* count, int flags);
/// Start streaming.
int me6000_ao_io_stream_start(me_subdevice_t* subdevice, struct file* filep, int start_mode, int time_out, int flags);
/// Check actual state. / Wait for end.
int me6000_ao_io_stream_status(me_subdevice_t* subdevice, struct file* filep, int wait, int* status, int* values, int flags);
/// Stop streaming.
int me6000_ao_io_stream_stop(me_subdevice_t* subdevice, struct file* filep, int stop_mode, int time_out, int flags);
/// Write datas to buffor.
int me6000_ao_io_stream_write(me_subdevice_t* subdevice, struct file* filep, int write_mode, int* values, int* count, int time_out, int flags);
/// Interrupt handler. Copy from buffer to FIFO.
static int me6000_ao_irq_handle(me_subdevice_t* subdevice, uint32_t irq_status);
/// Copy data from circular buffer to fifo (fast) in wraparound mode.
int inline ao_write_data_wraparound(me6000_ao_subdevice_t* instance, int count, int start_pos);
/// Copy data from circular buffer to fifo (fast).
int inline ao_write_data(me6000_ao_subdevice_t* instance, int count, int start_pos);
/// Copy data from circular buffer to fifo (slow).
int inline ao_write_data_pooling(me6000_ao_subdevice_t* instance, int count, int start_pos);
/// Copy data from user space to circular buffer.
int inline ao_get_data_from_user(me6000_ao_subdevice_t* instance, int count, int* user_values);
...
static int inline ao_isnt_fifo_full(me6000_ao_subdevice_t* instance);
static void ao_determine_FIFO_size(me6000_ao_subdevice_t* instance);
	...
	ME_SUBDEVICE_ENTER;
		ME_LOCK_PROTECTOR;
			...
			ME_SPIN_LOCK(instance->preload_reg_lock);
				...
				tmp &= ~((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx);
				me_writel(instance->base.dev, tmp, instance->preload_reg);
				*instance->preload_flags &= ~((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx);

				//Reset triggering flag
				*instance->triggering_flags &= ~(0x1 << instance->base.idx);
			...
			if (instance->fifo)
			{
				...
				//Set speed
				me_writel(instance->base.dev, ME6000_AO_MIN_CHAN_TICKS-1, instance->timer_reg);
				//Reset interrupt latch
				me_readl(instance->base.dev, &tmp, instance->irq_reset_reg);
			}
    									...
    									int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags)
{
	...
	switch (trig_type)
	{
		...
		case ME_TRIG_TYPE_EXT_DIGITAL:
				if (instance->fifo)
				{// Stream hardware (with or without fifo).
					switch(trig_edge)
					{
						...
						default:
							PERROR("Invalid trigger edge. Must be ME_TRIG_EDGE_RISING, ME_TRIG_EDGE_FALLING or ME_TRIG_EDGE_ANY.\n");
							return ME_ERRNO_INVALID_TRIG_EDGE;
					}
				}
				else
				{	// Single hardware only.
					PERROR("Invalid trigger type. Should be ME_TRIG_TYPE_SW.\n");
	...
	{
		PERROR("Invalid reference specified. Must be REF_AO_GROUND.\n");
    									...
    									int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags)
{
	...
	// Checking parameters
	err = me6000_ao_io_single_config_check(instance, channel, single_config, ref, trig_chain, trig_type, trig_edge, flags);
    if (err)
    	return err;
	...
	ME_SUBDEVICE_ENTER;
		ME_SUBDEVICE_LOCK;
			...
			{// Set control register.
				...
				//Reset all bits.
				tmp = ME6000_AO_CTRL_BIT_IMMEDIATE_STOP | ME6000_AO_CTRL_BIT_STOP;
				if (trig_type == ME_TRIG_TYPE_EXT_DIGITAL)
				{
					...
					{
						instance->ctrl_trg = ME6000_AO_CTRL_BIT_EX_TRIG_EDGE | ME6000_AO_CTRL_BIT_EX_TRIG_EDGE_BOTH;
					}
			...
			ME_SPIN_LOCK(instance->preload_reg_lock);
				if (trig_type == ME_TRIG_TYPE_EXT_DIGITAL)
				{
					*instance->preload_flags |= ME6000_AO_SYNC_EXT_TRIG << instance->base.idx;
				}
				else	//if (trig_type == ME_TRIG_TYPE_SW)
				{
					*instance->preload_flags &= ~(ME6000_AO_SYNC_EXT_TRIG << instance->base.idx);
				...
				{
					*instance->preload_flags |= ME6000_AO_SYNC_HOLD << instance->base.idx;
				}
				...
				me_readl(instance->base.dev, &tmp, instance->preload_reg);
				tmp &= ~(ME6000_AO_SYNC_EXT_TRIG << instance->base.idx);
				tmp |= ME6000_AO_SYNC_HOLD << instance->base.idx;
...
int me6000_ao_io_single_read(me_subdevice_t* subdevice, struct file* filep, int channel, int* value, int time_out, int flags)
{
	...
	err = me6000_ao_io_single_read_check(instance, channel, value, time_out, flags);
    if (err)
    	return err;
...
static int me6000_ao_io_single_write_check(me6000_ao_subdevice_t* instance, int channel, int value, int time_out, int flags)
{
	if (flags & ~(ME_IO_SINGLE_TYPE_TRIG_SYNCHRONOUS | ME_IO_SINGLE_TYPE_NONBLOCKING))
	{
		PERROR("Invalid flag specified. Must be ME_IO_SINGLE_TYPE_NO_FLAGS, ME_IO_SINGLE_TYPE_NONBLOCKING or ME_IO_SINGLE_TYPE_TRIG_SYNCHRONOUS.\n");
		return ME_ERRNO_INVALID_FLAGS;
	}
...
/// Workaround for mix-mode - end
	...
	err = me6000_ao_io_single_write_check(instance, channel, value, time_out, flags);
    if (err)
    	return err;
	...
	ME_SUBDEVICE_ENTER;
		...
		{/// Workaround for mix-mode - begin
			...
			//Prepare FIFO
			if (!(ctrl & ME6000_AO_CTRL_BIT_ENABLE_FIFO))
			...
			{//Check if FIFO is empty
				if (status & ME6000_AO_STATUS_BIT_EF)
				{//FIFO not empty
					PINFO("Reseting FIFO.\n");
					ctrl &= ~(ME6000_AO_CTRL_BIT_ENABLE_FIFO | ME6000_AO_CTRL_BIT_ENABLE_IRQ);
					me_writel(instance->base.dev, ctrl, instance->ctrl_reg);
			...
			PINFO("Write value: %x\n", value);
			me_writel(instance->base.dev, (value & 0xFFFF) | (value << 16), instance->fifo_reg);
			/// Workaround for mix-mode - end
		}
		else
		{//No FIFO - always in single mode
			//Write value
			PINFO("Write value: %x\n", value);
			me_writel(instance->base.dev, value, instance->single_reg);
		}
		...
		if (mode)
		{
			me_writel(instance->base.dev, ME6000_AO_OPTIMAL_CHAN_TICKS, instance->timer_reg);
		}
		else
		{
			me_writel(instance->base.dev, ME6000_AO_MIN_CHAN_TICKS-1, instance->timer_reg);
		...
		ME_SPIN_LOCK(instance->preload_reg_lock);
			...
			switch(mode)
			{
				case 0:							//0x00000000: Individual software
					...
					if (!(instance->fifo & ME6000_AO_HAS_FIFO))
					{// No FIFO - Single mode: In this case resetting 'ME6000_AO_SYNC_HOLD' will trigger output.
						if ((sync_mask & ((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx)) != ME6000_AO_SYNC_HOLD)
						{//Now we can set correct mode. This is exception. It is set to synchronous and triggered later.
							sync_mask &= ~(ME6000_AO_SYNC_EXT_TRIG << instance->base.idx);
							sync_mask |= ME6000_AO_SYNC_HOLD << instance->base.idx;
							me_writel(instance->base.dev, sync_mask, instance->preload_reg);
						}
					}
					else
					{// FIFO - Continous mode
						if((sync_mask & ((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx)) != 0x0)
						{//Now we can set correct mode.
							sync_mask &= ~((ME6000_AO_SYNC_EXT_TRIG | ME6000_AO_SYNC_HOLD) << instance->base.idx);
							me_writel(instance->base.dev, sync_mask, instance->preload_reg);
						}
					}
				...
				case ME6000_AO_SYNC_HOLD:				//0x00000001: Synchronous software
					...
					if ((sync_mask & ((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx)) != (ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG))
					{ //Now we can set correct mode
						sync_mask |= ME6000_AO_SYNC_EXT_TRIG << instance->base.idx;
						sync_mask |= ME6000_AO_SYNC_HOLD << instance->base.idx;
						me_writel(instance->base.dev, sync_mask, instance->preload_reg);
				...
				case (ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG): 	//0x00010001: Synchronous hardware
					...
					if ((sync_mask & ((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx)) != (ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG))
					{ //Now we can set correct mode
						sync_mask |= (ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx;
						me_writel(instance->base.dev, sync_mask, instance->preload_reg);
					}
					//Set triggering flag
					*instance->triggering_flags |= 0x1 << instance->base.idx;
			...
			{// No FIFO - Single mode
				...
				{// Add this channel to list
					me_writel(instance->base.dev, sync_mask & ~(ME6000_AO_SYNC_HOLD << instance->base.idx), instance->preload_reg);
		...
		*/
		...
		if (instance->status != ao_status_single_end)
		{
			...
			queue_delayed_work(instance->me6000_workqueue, &instance->ao_control_task, 1);
				...
				else
				{
					switch (instance->status)
					{
						...
						case ao_status_single_run:
							...
							me_readl(instance->base.dev, &ctrl, instance->ctrl_reg);
							ctrl |= ME6000_AO_CTRL_BIT_STOP | ME6000_AO_CTRL_BIT_IMMEDIATE_STOP;
							ctrl &= ~(ME6000_AO_CTRL_BIT_ENABLE_IRQ | ME6000_AO_CTRL_BIT_ENABLE_EX_TRIG);
							ctrl &= ~(ME6000_AO_CTRL_BIT_EX_TRIG_EDGE | ME6000_AO_CTRL_BIT_EX_TRIG_EDGE_BOTH);
							if (instance->fifo & ME6000_AO_HAS_FIFO)
							{//Disabling FIFO
								ctrl &= ~ME6000_AO_CTRL_BIT_ENABLE_FIFO;
							...
							ME_SPIN_LOCK(instance->preload_reg_lock);
								...
								synch &= ~(ME6000_AO_SYNC_EXT_TRIG << instance->base.idx);
								synch |= ME6000_AO_SYNC_HOLD << instance->base.idx;
    									...
    									meIOStreamSimpleConfig_t* config_list, int count, meIOStreamSimpleTriggers_t* trigger, int fifo_irq_threshold, int flags)
{
	...
	if (flags & ~(ME_IO_STREAM_CONFIG_HARDWARE_ONLY | ME_IO_STREAM_CONFIG_WRAPAROUND))
	{
	...
	if ((trigger->conv_ticks < ME6000_AO_MIN_CHAN_TICKS) || (trigger->conv_ticks > ME6000_AO_MAX_CHAN_TICKS))
	{
		PERROR("Invalid conv start trigger argument specified. Must be between %lld and %lld.\n", ME6000_AO_MIN_CHAN_TICKS, ME6000_AO_MAX_CHAN_TICKS);
    									...
    									meIOStreamSimpleConfig_t* config_list, int count, meIOStreamSimpleTriggers_t* trigger, int fifo_irq_threshold, int flags)
{
	...
	err = me6000_ao_io_stream_config_check(instance, config_list, count, trigger, fifo_irq_threshold, flags);
    if (err)
    	return err;
	...
	ME_SUBDEVICE_ENTER;
		ME_LOCK_PROTECTOR
			...
			{// Continous
				PINFO("Continous.\n");
				ctrl |= ME6000_AO_MODE_CONTINUOUS;
				instance->mode = ME6000_AO_CONTINUOUS;
			}
			me_writel(instance->base.dev, ctrl, instance->ctrl_reg);
			...
			switch (trigger->stop_type)
			{
				...
				case ME_STREAM_STOP_TYPE_ACQ_LIST:
				instance->stop_mode = ME6000_AO_LIST_STOP_MODE;
				instance->stop_count = trigger->stop_count;
					break;
			...
			{// Synchronous start
				...
				{
					PINFO("Synchronous start. Externaly trigger dissabled.\n");
				}
			}
...
int me6000_ao_io_stream_start(me_subdevice_t* subdevice, struct file* filep, int start_mode, int time_out, int flags)
{
	...
	ME_SUBDEVICE_ENTER;
		ME_LOCK_PROTECTOR;
			...
			if (instance->mode == ME6000_AO_CONTINUOUS)
			{//Continous
			...
			//Setting stop points
			if (instance->stop_mode == ME6000_AO_LIST_STOP_MODE)
			{
			...
			// Set ctrl
			ctrl = instance->ctrl_trg & ~(ME6000_AO_CTRL_BIT_EX_TRIG_EDGE | ME6000_AO_CTRL_BIT_EX_TRIG_EDGE_BOTH);
			//Block IRQ
			ctrl &= ~ME6000_AO_CTRL_BIT_ENABLE_IRQ;
			ctrl |= ME6000_AO_CTRL_BIT_ENABLE_FIFO;
			me_writel(instance->base.dev, ctrl, instance->ctrl_reg);
			...
			ME_SPIN_LOCK(instance->preload_reg_lock);
				me_readl(instance->base.dev, &synch, instance->preload_reg);
				synch &= ~((ME6000_AO_SYNC_HOLD | ME6000_AO_SYNC_EXT_TRIG) << instance->base.idx);
				synch |= (instance->start_mode & ~ME6000_AO_EXT_TRIG) << instance->base.idx;
				me_writel(instance->base.dev, synch, instance->preload_reg);
			ME_SPIN_UNLOCK(instance->preload_reg_lock);
			...
			else if ((instance->mode == ME6000_AO_SW_WRAP_MODE) || (instance->mode == ME6000_AO_HW_WRAP_MODE))
			{//Wraparound
			...
			if (!(status & ME6000_AO_STATUS_BIT_HF))
			{//More than half!
				if ((ctrl & ME6000_AO_CTRL_MODE_MASK) == ME6000_AO_MODE_CONTINUOUS)
			...
			{//Trigger outputs
				ME_SPIN_LOCK(instance->preload_reg_lock);
					me_readl(instance->base.dev, &synch, instance->preload_reg);
					//Add channel to start list
					me_writel(instance->base.dev, synch | (ME6000_AO_SYNC_HOLD << instance->base.idx), instance->preload_reg);
		...
		/*
				ME_SPIN_LOCK(instance->preload_reg_lock);
					synch = me_readl(instance->preload_reg);
					//Remove channel from start list
					me_writel(instance->base.dev, synch & ~(ME6000_AO_SYNC_HOLD << instance->base.idx), instance->preload_reg);
		*/
					//Fire
		...
		*/
		...
		if ((instance->mode == ME6000_AO_CONTINUOUS) || (instance->mode == ME6000_AO_SW_WRAP_MODE))
		{
			if (status & ME6000_AO_STATUS_BIT_HF)
			{//Less than half but not empty!
				...
				if (instance->stop_data_count == 0)
				{
					count = instance->fifo_size/2;
				}
				else
				{
					count = ((instance->fifo_size>>1) < instance->stop_data_count) ? (instance->fifo_size>>1) : instance->stop_data_count;
				}
				...
				else if (instance->mode == ME6000_AO_SW_WRAP_MODE)
				{//Wraparound
					instance->data_count += count;
					instance->preloaded_count += count;
			...
			if ((instance->stop_mode != ME6000_AO_INF_STOP_MODE)
				...
				(instance->mode == ME6000_AO_SW_WRAP_MODE)
					&&
				(circ_buffer_count <= (instance->fifo_size>>1)))
			{//Put more data to FIFO
				...
				{//Maximum data not set jet.
					//Copy to buffer
					if(circ_buffer_count != ao_write_data(instance, circ_buffer_count, 0))
					{//This should never happend!
						PERROR_CRITICAL("ERROR WHEN LOADING VALUES FOR WRAPAROUND!\n");
						ME_SUBDEVICE_UNLOCK;
						ME_SUBDEVICE_EXIT;
...
int me6000_ao_io_stream_status(me_subdevice_t* subdevice, struct file* filep, int wait, int* status, int* values, int flags)
{
	...
	switch (instance->status)
	{
		...
		default:
			me_readl(instance->base.dev, &tmp, instance->status_reg);
			*status = (tmp & ME6000_AO_STATUS_BIT_FSM) ? ME_STATUS_BUSY : ME_STATUS_IDLE;
			break;
	}
...
{// Stop work and empty buffer and FIFO
	...
	ME_SUBDEVICE_ENTER;
		ME_LOCK_PROTECTOR;
			...
			{
				me_readl(instance->base.dev, &tmp, instance->ctrl_reg);
				if ((tmp & ME6000_AO_CTRL_MODE_MASK) == ME6000_AO_MODE_WRAPAROUND)
				{//Hardware wraparound => Hardware stop.
					tmp |= ME6000_AO_CTRL_BIT_STOP;
					tmp &= ~ME6000_AO_CTRL_BIT_ENABLE_IRQ;
					me_writel(instance->base.dev, tmp, instance->ctrl_reg);
...
END:
			me_readl(instance->base.dev, &tmp, instance->ctrl_reg);
			tmp |= ME6000_AO_CTRL_BIT_STOP | ME6000_AO_CTRL_BIT_IMMEDIATE_STOP;
			tmp &= ~(ME6000_AO_CTRL_BIT_ENABLE_EX_TRIG | ME6000_AO_CTRL_BIT_EX_TRIG_EDGE | ME6000_AO_CTRL_BIT_EX_TRIG_EDGE_BOTH);
			tmp &= ~ME6000_AO_CTRL_BIT_ENABLE_IRQ;
			if (!flags)
			{//Reset FIFO
				tmp &= ~ME6000_AO_CTRL_BIT_ENABLE_FIFO;
			}
			me_writel(instance->base.dev, tmp, instance->ctrl_reg);
...
/// @note If no 'pre-load' is used. stream_start() will move data to FIFO.
	...
	if (instance->mode & ME6000_AO_WRAP_MODE)
	...
	if ((instance->mode == ME6000_AO_HW_WRAP_MODE) && (write_mode != ME_WRITE_MODE_PRELOAD))
	...
	ME_SUBDEVICE_ENTER;
		ME_LOCK_PROTECTOR;
			...
			{//Init enviroment - preload
				me_readl(instance->base.dev, &tmp, instance->ctrl_reg);
				//Check FIFO
				if (!(tmp & ME6000_AO_CTRL_BIT_ENABLE_FIFO))
			...
			while(!err)
			{
				...
				me_readl(instance->base.dev, &tmp, instance->status_reg);
				if ((instance->status == ao_status_stream_run) && !(tmp & ME6000_AO_STATUS_BIT_FSM))
				...
				if ((instance->status == ao_status_stream_run) && (instance->mode == ME6000_AO_CONTINUOUS)  && (tmp & ME6000_AO_STATUS_BIT_HF))
				{//Continous mode runing and data are below half!
					...
					//Fast copy
					copied_values = ao_write_data(instance, instance->fifo_size>>1, 0);
					if (copied_values > 0)
					{
			...
			if (!err)
			{
				...
				{//Copy data to FIFO - preload
					...
					if ((instance->mode == ME6000_AO_HW_WRAP_MODE) && (me_circ_buf_values(&instance->circ_buf) > instance->fifo_size))
					{
...
static int me6000_ao_irq_handle(me_subdevice_t* subdevice, uint32_t irq_status)
{
	me6000_ao_subdevice_t *instance = (me6000_ao_subdevice_t *)subdevice;
	uint32_t ctrl;
	uint32_t status;
	uint32_t tmp;
	int count = 0;
	...
	ME_HANDLER_PROTECTOR;
		...
		me_readl(instance->base.dev, &status, instance->status_reg);
		if (!(status & ME6000_AO_STATUS_BIT_FSM))
		...
		{//OK less than half
			...
			do
			{
				...
				else if ((instance->mode == ME6000_AO_SW_WRAP_MODE) && ((ctrl & ME6000_AO_CTRL_MODE_MASK) == ME6000_AO_MODE_CONTINUOUS))
				{//Wraparound (software)
					if (instance->status == ao_status_stream_end_wait)
			...
			while ((status = me6000_ao_FSM_test(instance)) & ME6000_AO_STATUS_BIT_HF);
			...
			{//Copy was successful.
				...
				{//Normal work but there are no more data in buffer. Interrupt blocked. stream_write() will unblock it.
...
static void me6000_ao_destructor(me_subdevice_t* subdevice)
{
	...
	if (((me_general_dev_t *)instance->base.dev)->dev)
	{
		...
		me6000_ao_io_reset_subdevice(subdevice, NULL, ME_IO_RESET_SUBDEVICE_NO_FLAGS);
	}
	...
	if (instance->fifo)
	{
		if(instance->circ_buf.buf)
		{
			PDEBUG("free circ_buf = %p size=%d\n", instance->circ_buf.buf, PAGE_SHIFT<<ME6000_AO_CIRC_BUF_SIZE_ORDER);
			free_pages((unsigned long)instance->circ_buf.buf, ME6000_AO_CIRC_BUF_SIZE_ORDER);
		}
...
int inline ao_stop_immediately(me6000_ao_subdevice_t *instance)
{
	...
	single_mask = (instance->base.idx-ME6000_AO_SINGLE_STATUS_OFFSET < 0) ? 0x0000 : (0x0001 << (instance->base.idx-ME6000_AO_SINGLE_STATUS_OFFSET));
	if (instance->fifo)
	{
		// Stop all actions. No conditions! Block interrupts. Leave FIFO untouched!
		me_readl(instance->base.dev, &tmp, instance->ctrl_reg);
		tmp |= ME6000_AO_CTRL_BIT_STOP | ME6000_AO_CTRL_BIT_IMMEDIATE_STOP;
		tmp &= ~(ME6000_AO_CTRL_BIT_ENABLE_IRQ | ME6000_AO_CTRL_BIT_ENABLE_EX_TRIG);
		me_writel(instance->base.dev, tmp, instance->ctrl_reg);
	}
...
* @note This is soft time critical function. Checking is done at begining and end only.
* @note The is not reasonable way to check how many walues was in FIFO at begining. The count of it must be managed externaly.
*
* @param instance The subdevice instance (pointer).
* @param count Maximum number of copied data.
* @param start_pos Position of the first value in buffer.
*
...
#if defined(ME_USB) && defined(ME_USE_DMA)
{
	...
	while(i < local_count)
	{
		...
		buffer[i] = (value & 0xFFFF) | (value << 16);
	...
	if (local_count && !err)
	{
		...
		if (!err)
		{
			if (!(status & ME6000_AO_STATUS_BIT_FF))
			...
			{//Add last value
				value = buffer[local_count-1];

				//Put value to FIFO
				me_writel(instance->base.dev, value, instance->fifo_reg);
			}
		}
	}
...
{/// @note This is slow function!
	...
	if (count <= 0)
	{//Wrong count!
		PERROR("idx=%d SLOW: UPLOADED: Wrong count!\n", instance->base.idx);
		return 0;
	}
	...
	if (max_count <= 0)
	{//No data to copy!
		PERROR("idx=%d SLOW: UPLOADED: No data to copy!\n", instance->base.idx);
		return 0;
	}
	...
	for (i=0; i<local_count; i++)
	{
		...
		if (!(status & ME6000_AO_STATUS_BIT_FF))
		{//FIFO is full!
			PINFO("idx=%d FIFO is full! SLOW: LOADED %d values\n", instance->base.idx, i);
			return i;
		}
	...
	PINFO("idx=%d SLOW: UPLOADED %d values\n", instance->base.idx, local_count);
	return local_count;
}
...
int inline ao_get_data_from_user(me6000_ao_subdevice_t* instance, int count, int* user_values)
{
	...
	{//Copy from user to buffer
		...
		{
			PERROR("idx=%d BUFFER LOADED: get_user(0x%p) return an error: %d\n", instance->base.idx, user_values + i, err);
			return -ME_ERRNO_INTERNAL;
		}
										...
										)
{
	...
	instance = (me6000_ao_subdevice_t *) subdevice;
#else
	instance = container_of((void *)work, me6000_ao_subdevice_t, ao_control_task);
#endif
...
/// @note AO_STATUS_BIT_FSM doesn't work as should be for pure single channels (idx>=4)
		single_mask = *instance->triggering_flags & (0x1 << instance->base.idx);
			...
			case ao_status_single_run:
				...
				{// Extra registers.
					...
					{// State machine is not working.
						if (((instance->fifo & ME6000_AO_HAS_FIFO) && (!(status & ME6000_AO_STATUS_BIT_EF)))
							||
							(!(instance->fifo & ME6000_AO_HAS_FIFO)))
						{// Single is in end state.
							...
							me_writel(instance->base.dev, ctrl, instance->ctrl_reg);
							//Reset interrupt latch
							me_readl(instance->base.dev, &ctrl, instance->irq_reset_reg);
							...
							ME_SPIN_LOCK(instance->preload_reg_lock);
								if ((single_mask) && (*instance->preload_flags & (ME6000_AO_SYNC_HOLD << instance->base.idx)))
								{// This is one of synchronous start channels. Set all as triggered.
										*instance->triggering_flags = 0x00000000;
								}
								else
								{
									//Set this channel as triggered (none active).
...
int me6000_ao_query_subdevice_type(me_subdevice_t* subdevice, int* type, int* subtype)
{
	...
	*type = ME_TYPE_AO;
	*subtype = (instance->fifo & ME6000_AO_HAS_FIFO) ? ME_SUBTYPE_STREAMING : ME_SUBTYPE_SINGLE;
	...
	*count = 1;
	switch (cap)
	{
		...
		case ME_CAP_AO_BUFFER_SIZE:
			*args = (instance->circ_buf.buf) ? ME6000_AO_CIRC_BUF_COUNT : 0;
		break;
...
int me6000_ao_postinit(me_subdevice_t* subdevice, void* args)
{
	ao_determine_FIFO_size((me6000_ao_subdevice_t *)subdevice);

	// Reset subdevice.
	return me6000_ao_io_reset_subdevice(subdevice, NULL, ME_IO_RESET_SUBDEVICE_NO_FLAGS);
}
...
me6000_ao_subdevice_t* me6000_ao_constr(void* reg_base,
											...
											int fifo, int high_range, struct workqueue_struct* me6000_wq)
{
	...
	subdevice = kzalloc(sizeof(me6000_ao_subdevice_t), GFP_KERNEL);
	if (!subdevice)
	{
	...
	if(subdevice->fifo & ME6000_AO_HAS_FIFO)
	{
		...
		subdevice->circ_buf.buf = (void *)__get_free_pages(GFP_KERNEL, ME6000_AO_CIRC_BUF_SIZE_ORDER);
		...
		PINFO("circ_buf = %p size=%ld\n", subdevice->circ_buf.buf, ME6000_AO_CIRC_BUF_SIZE);

		memset(subdevice->circ_buf.buf, 0, ME6000_AO_CIRC_BUF_SIZE);

		subdevice->fifo_size = ME6000_AO_FIFO_COUNT;
	}
	else
	{
	...
	if (idx < 4)
	{
		subdevice->ctrl_reg =		reg_base + ME6000_AO_CTRL_REG + (ME6000_AO_PORT_OFFSET * idx);
		subdevice->status_reg =		reg_base + ME6000_AO_STATUS_REG + (ME6000_AO_PORT_OFFSET * idx);
		subdevice->fifo_reg =		reg_base + ME6000_AO_FIFO_REG + (ME6000_AO_PORT_OFFSET * idx);
		subdevice->timer_reg =		reg_base + ME6000_AO_TIMER_REG + (ME6000_AO_PORT_OFFSET * idx);
		subdevice->single_reg =		reg_base + ME6000_AO_SINGLE_REG + (ME6000_AO_PORT_OFFSET * idx);
		subdevice->irq_reset_reg =	reg_base + ME6000_AO_IRQ_RESET_REG + (ME6000_AO_IRQ_RESET_OFFSET * idx);
	}
	else if (idx < 16)
	{
		...
		subdevice->single_reg =		reg_base + ME6000_AO_SINGLE_SINGLE_REG + (ME6000_AO_SINGLE_PORT_OFFSET * (idx - 4));
	}
	else
	{
		PERROR_CRITICAL("WRONG SUBDEVICE ID=%d!", idx);
		...
		if(subdevice->fifo)
		{
			free_pages((unsigned long)subdevice->circ_buf.buf, ME6000_AO_CIRC_BUF_SIZE_ORDER);
		}
		subdevice->circ_buf.buf = NULL;
		kfree(subdevice);
		return NULL;
	...
	subdevice->base.me_subdevice_io_reset_subdevice = me6000_ao_io_reset_subdevice;
	subdevice->base.me_subdevice_io_single_config = me6000_ao_io_single_config;
	subdevice->base.me_subdevice_io_single_read = me6000_ao_io_single_read;
	subdevice->base.me_subdevice_io_single_write = me6000_ao_io_single_write;
	subdevice->base.me_subdevice_io_stream_config = me6000_ao_io_stream_config;
	subdevice->base.me_subdevice_io_stream_new_values = me6000_ao_io_stream_new_values;
	subdevice->base.me_subdevice_io_stream_write = me6000_ao_io_stream_write;
	subdevice->base.me_subdevice_io_stream_start = me6000_ao_io_stream_start;
	subdevice->base.me_subdevice_io_stream_status = me6000_ao_io_stream_status;
	subdevice->base.me_subdevice_io_stream_stop = me6000_ao_io_stream_stop;
	subdevice->base.me_subdevice_query_number_channels = me6000_ao_query_number_channels;
	subdevice->base.me_subdevice_query_subdevice_type = me6000_ao_query_subdevice_type;
	subdevice->base.me_subdevice_query_subdevice_caps = me6000_ao_query_subdevice_caps;
	subdevice->base.me_subdevice_query_subdevice_caps_args = me6000_ao_query_subdevice_caps_args;
	subdevice->base.me_subdevice_query_range_by_min_max = me6000_ao_query_range_by_min_max;
	subdevice->base.me_subdevice_query_number_ranges = me6000_ao_query_number_ranges;
	subdevice->base.me_subdevice_query_range_info = me6000_ao_query_range_info;
	subdevice->base.me_subdevice_query_timer = me6000_ao_query_timer;

	subdevice->base.me_subdevice_postinit = me6000_ao_postinit;
	subdevice->base.me_subdevice_irq_handle = me6000_ao_irq_handle;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
	INIT_WORK(&subdevice->ao_control_task, me6000_ao_work_control_task, (void *)subdevice);
#else
	INIT_DELAYED_WORK(&subdevice->ao_control_task, me6000_ao_work_control_task);
#endif
...
static int inline ao_isnt_fifo_full(me6000_ao_subdevice_t* instance)
{
	uint32_t tmp;
	me_readl(instance->base.dev, &tmp, instance->status_reg);
	return tmp & ME6000_AO_STATUS_BIT_FF;
}
...
static void ao_determine_FIFO_size(me6000_ao_subdevice_t* instance)
{
	...
	ME_LOCK_PROTECTOR;
		...
		ctrl = (0x0000
			| ME6000_AO_CTRL_BIT_MODE_WRAPAROUND
