 ...
 * @file me0900_di.c
 *
 * @brief The ME-0900 (ME-9x) digital input subdevice instance.
...
int me0900_di_io_single_config(me_subdevice_t* subdevice, struct file* filep, int channel,
											int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags);
int me0900_di_io_single_read(me_subdevice_t* subdevice, struct file* filep, int channel, int* value, int time_out, int flags);
int me0900_di_query_number_channels(me_subdevice_t* subdevice, int* number);
int me0900_di_query_subdevice_type(me_subdevice_t* subdevice, int* type, int* subtype);
int me0900_di_query_subdevice_caps(me_subdevice_t* subdevice, int* caps);
											...
											int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags)
{
	...
	{
		PERROR("Invalid byte direction specified. Must be ME_SINGLE_CONFIG_DIO_INPUT.\n");
...
me0900_di_subdevice_t* me0900_di_constr(void* reg_base, unsigned int idx, int start_port)
{
	...
	subdevice = kzalloc(sizeof(me0900_di_subdevice_t), GFP_KERNEL);
	if (!subdevice)
	{
	...
	subdevice->port_reg = reg_base + ME0900_PORT_REG + ((start_port + idx) * ME0900_PORT_SHIFT);

	/* Overload base class methods. */
	subdevice->base.me_subdevice_io_single_config = me0900_di_io_single_config;
	subdevice->base.me_subdevice_io_single_read = me0900_di_io_single_read;
	subdevice->base.me_subdevice_query_number_channels = me0900_di_query_number_channels;
	subdevice->base.me_subdevice_query_subdevice_type = me0900_di_query_subdevice_type;
	subdevice->base.me_subdevice_query_subdevice_caps = me0900_di_query_subdevice_caps;
