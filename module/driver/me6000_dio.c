 ...
 * @file me6000_dio.c
 *
 * @brief The ME-6000 digital input/output subdevice instance.
...
int me6000_dio_io_reset_subdevice(me_subdevice_t* subdevice, struct file* filep, int flags);
int me6000_dio_io_single_config(me_subdevice_t* subdevice, struct file* filep, int channel,
										int single_config, int ref, int trig_chain, int trig_type, int trig_edge, int flags);
int me6000_dio_io_single_read(me_subdevice_t* subdevice, struct file* filep, int channel, int* value, int time_out, int flags);
int me6000_dio_io_single_write(me_subdevice_t* subdevice, struct file* filep, int channel, int value, int time_out, int flags);
int me6000_dio_query_number_channels(me_subdevice_t* subdevice, int* number);
int me6000_dio_query_subdevice_type(me_subdevice_t* subdevice, int* type, int* subtype);
int me6000_dio_query_subdevice_caps(me_subdevice_t* subdevice, int* caps);
	...
	ME_SUBDEVICE_ENTER;
		ME_SUBDEVICE_LOCK;
			ME_SPIN_LOCK(instance->ctrl_reg_lock);
				...
				tmp &= ~(ME6000_DIO_CTRL_BIT_MODE_MASK << (instance->base.idx * ME6000_DIO_MODE_SHIFT));
	...
	ME_SUBDEVICE_ENTER
		ME_SUBDEVICE_LOCK;
			ME_SPIN_LOCK(instance->ctrl_reg_lock);
				...
				{
					tmp |= ME6000_DIO_CTRL_BIT_MODE_OUTPUT << (instance->base.idx * ME6000_DIO_MODE_SHIFT);
				}
				me_writeb(instance->base.dev, tmp, instance->ctrl_reg);
			ME_SPIN_UNLOCK(instance->ctrl_reg_lock);
		ME_SUBDEVICE_UNLOCK;
...
int me6000_dio_io_single_read(me_subdevice_t* subdevice, struct file* filep, int channel, int* value, int time_out, int flags)
{
	...
	ME_SUBDEVICE_ENTER
		...
		me_readb(instance->base.dev, &tmp, instance->ctrl_reg);
		tmp >>= instance->base.idx * ME6000_DIO_MODE_SHIFT;
		tmp &= ME6000_DIO_CTRL_BIT_MODE_MASK;

		if ((tmp != ME6000_DIO_CTRL_BIT_MODE_OUTPUT) && (tmp != ME6000_DIO_CTRL_BIT_MODE_INPUT))
		{
			PERROR_CRITICAL("Port not in output or input mode.\n");
...
int me6000_dio_io_single_write(me_subdevice_t* subdevice, struct file* filep, int channel, int value, int time_out, int flags)
{
	...
	ME_SUBDEVICE_ENTER
 		ME_SUBDEVICE_LOCK;
			me_readb(instance->base.dev, &tmp, instance->ctrl_reg);
			tmp >>= (instance->base.idx << (instance->base.idx * ME6000_DIO_MODE_SHIFT));
			tmp &= ME6000_DIO_CTRL_BIT_MODE_MASK;

			if (tmp != ME6000_DIO_CTRL_BIT_MODE_OUTPUT)
			{
...
int me6000_dio_query_number_channels(me_subdevice_t* subdevice, int* number)
{
	PDEBUG("executed idx=%d.\n", ((me6000_dio_subdevice_t *) subdevice)->base.idx);
...
int me6000_dio_query_subdevice_caps(me_subdevice_t* subdevice, int* caps)
{
	...
	*caps = me6000_DIO_CAPS;	//ME_CAPS_DIO_DIR_BYTE;

	return ME_ERRNO_SUCCESS;
}

me6000_dio_subdevice_t* me6000_dio_constr(void* reg_base, unsigned int idx, me_lock_t* ctrl_reg_lock)
{
	...
	subdevice = kzalloc(sizeof(me6000_dio_subdevice_t), GFP_KERNEL);
	if (!subdevice)
	{
	...
	subdevice->port_reg = reg_base + ME6000_DIO_PORT_REG + (idx * ME6000_PORT_STEP);


	// Override base class methods.
	subdevice->base.me_subdevice_io_reset_subdevice = me6000_dio_io_reset_subdevice;
	subdevice->base.me_subdevice_io_single_config = me6000_dio_io_single_config;
	subdevice->base.me_subdevice_io_single_read = me6000_dio_io_single_read;
	subdevice->base.me_subdevice_io_single_write = me6000_dio_io_single_write;
	subdevice->base.me_subdevice_query_number_channels = me6000_dio_query_number_channels;
	subdevice->base.me_subdevice_query_subdevice_type = me6000_dio_query_subdevice_type;
	subdevice->base.me_subdevice_query_subdevice_caps = me6000_dio_query_subdevice_caps;
