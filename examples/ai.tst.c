...
// 	Trigger.iConvStartTicksLow = 66;	//500KHz
	...
	printf("Hello, AI test (NON BLOCKING MODE)!\n");
	...
	{
		ret_err = meIOStreamNewValues(NoDev, NoSubDev, 0, &AIBufferCount, ME_VALUE_NOT_USED);
		if (ret_err)
		{
			printf("EXIT %d\n", __LINE__);
			goto EXIT;
		...
		ret_err = meIOStreamRead(NoDev, NoSubDev, ME_READ_MODE_NONBLOCKING, AIBuffer, &AIBufferCount, ME_VALUE_NOT_USED);
		if (ret_err)
		{
			printf("EXIT %d\n", __LINE__);
			goto EXIT;
		...
		if (Trigger.iConvStartTicksLow > 1600)
		{
