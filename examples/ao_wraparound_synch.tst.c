...
// 	Trigger.iConvStartTicksLow = 165;	//200KHz
	...
	printf("Hello, AO stream synchronous start test!\n");
	...
	for (i=0; i<4; ++i)
	{
		meIOStreamConfig(NoDev, NoSubDev + i, &ConfigList, 1, &Trigger, ME_VALUE_NOT_USED, ME_IO_STREAM_CONFIG_WRAPAROUND);
	}
	...
	StartList[3].iFlags = ME_IO_STREAM_START_TYPE_TRIG_SYNCHRONOUS;
...
EXIT:
	meClose(0);
	printf("Boodbye, AO stream synchronous start test!\n");
