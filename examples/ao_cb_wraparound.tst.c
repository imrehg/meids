...
#define AOBufSize	0x4000
	...
	Trigger.iConvStartTicksLow = 660;
	...
	printf("Hello, AO callback test for WRAPAROUND mode!\n");
	...
	ret_err = meIOStreamConfig(NoDev, NoSubDev, &ConfigList, 1, &Trigger, ME_VALUE_NOT_USED, ME_IO_STREAM_CONFIG_WRAPAROUND);
	if (ret_err)
	{
		goto EXIT;
	}
	...
	printf("Write stream\n");
	ret_err = meIOStreamWrite(NoDev, NoSubDev, ME_WRITE_MODE_PRELOAD, AOBuffer, &AOBufferCount, ME_VALUE_NOT_USED);
	if (ret_err)
	{
		goto EXIT;
	}
...
EXIT:
	...
	sleep(1);
	meClose(0);
	printf("Boodbye, AO callback test for WRAPAROUND mode!\n");
